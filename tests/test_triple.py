...
def _meta_bytes(name: str) -> bytes:
    """Read a pinned golden copy from the META dir (the only valid source).

    The meta dir holds the cp'd seed elements whose sha256 matches the pin, so
    it is the canonical source for building a *valid* temp triple. It is
    deliberately NOT the live execution-plane source (which may drift).
    """
    return (TRIPLE_DIR / name).read_bytes()


def _skip_off_plane() -> None:
...
class TestVerifyTriple:
    def test_verify_passes_on_execution_plane(self, tmp_path: Path):
        # Build a faithful temp triple from the pinned meta copies (skip off-plane).
        _skip_off_plane()
        for name in TRIPLE:
            (tmp_path / name).write_bytes(_meta_bytes(name))
