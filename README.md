...
| `fixes.py` | Concrete per-failure-mode fix builders (`build_driver_death_fix`, `build_wall_kill_fix`, `build_stall_kill_fix`, `build_client_timeout_fix`, `build_inner_wall_fix`, `build_outer_freshness_fix`, `build_none_fix`) + the `FIX_BUILDERS` registry. The three original builders (driver-death, wall-kill, stall-kill) each emit a plan file PLUS a `cycles.out` marker file; the three derive-by-reference builders (client-timeout, inner-wall, outer-freshness) emit derived NEW-file variants; the healthy builder emits an empty path. |
